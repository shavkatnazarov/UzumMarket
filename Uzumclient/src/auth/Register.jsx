...
export const Registers = () => {
    ...
    const registers = async () => {
        const dto = {phoneNumber, name, password}
        try {
            await Register(dto)
        } catch (err) {
            console.log(err)
            ...
            <div>
                <div className={"container d-flex align-items-center justify-content-center"} style={{height: '100vh'}}>
                    <form className={"w-50 p-5 shadow"}>
                        ...
                        <div className="form-outline mb-4">
                            <input type="phoneNumber" placeholder={"tel raqam"} value={phoneNumber} onChange={e => setPhoneNumber(e.target.value)}
                                   id="form2Example1" className="form-control"/>
                            <label className="form-label" htmlFor="form2Example1">Telefon raqam</label>
                            <input placeholder={"parol"} type="phoneNumber" value={password} onChange={e => setPassword(e.target.value)}
                                   id="form2Example1" className="form-control"/>
